...
// import UsersPackagesGrid from "../../utlis/UsersPackageGrid";
// import { getUserProfile } from "../../api/authApi";
// import { initiateFund, transferFunds, withdrawal } from "../../api/walletApi";
...
//     setFundData((prev) => ({ ...prev, amount: MIN_AMOUNT.toString() }));
//   };
...
//                 }
//                 className="w-full p-2 pr-16 border rounded-md dark:bg-neutral-700 focus:ring-2 focus:ring-[#00A991] focus:border-transparent"
//                 placeholder={`Min ${MIN_AMOUNT} USDT`}
//               />
//               <button
//                 type="button"
//                 onClick={handleMinClick}
//                 className="absolute right-2 top-1/2 -translate-y-1/2 px-2 py-1 text-xs font-semibold text-[#00A991] bg-[#00A991]/10 rounded hover:bg-[#00A991]/20 transition"
//               >
//                 MIN
//               </button>
//             </div>
...
//               value={fundData.payCurrency}
//               onChange={(e) =>
//                 setFundData((prev) => ({ ...prev, payCurrency: e.target.value }))
//               }
//               className="w-full p-2 border rounded-md dark:bg-neutral-700"
//             >
//               <option value="USDTBSC">USDT (BSC)</option>
...
//                 <>
//                   <CheckCircle2 size={16} className="inline mr-1" />
//                   {fundResult.data?.message || "Success!"}  {/* Safe access */}
//                   <p className="mt-1 text-xs">
//                     Send {fundResult.data?.payAmount || 0} {fundResult.data?.payCurrency || "USDT"} to:
//                   </p>
//                   <p className="break-all font-mono text-xs">
//                     {fundResult.data?.payAddress || "No address provided"}
//                   </p>
//                 </>
//               ) : (
//                 <>
//                   <XCircle size={16} className="inline mr-1" />
//                   {fundResult.message || "Unknown error occurred."}  {/* Safe render */}
//                 </>
//               )}
//             </div>
//           )}
...
//             value={transferData.toUserId}
//             onChange={(e) =>
//               setTransferData((prev) => ({ ...prev, toUserId: e.target.value }))
//             }
//             className="w-full p-2 border rounded-md dark:bg-neutral-700"
...
//               <>
//                 <CheckCircle2 size={16} className="inline mr-1" />
//                 {transferResult.data?.message || "Transfer successful!"}  {/* Safe access */}
//               </>
//             ) : (
//               <>
//                 <XCircle size={16} className="inline mr-1" />
//                 {transferResult.message || "Transfer failed."}  {/* Safe render */}
//               </>
//             )}
//           </div>
//         )}
...
//               <>
//                 <CheckCircle2 size={16} className="inline mr-1" />
//                 {withdrawalResult.message || "Withdrawal submitted."}  {/* Safe render */}
//               </>
//             ) : (
//               <>
//                 <XCircle size={16} className="inline mr-1" />
//                 {withdrawalResult.message || "Withdrawal failed."}  {/* Safe render */}
//               </>
//             )}
//           </div>
//         )}
...
//   /* ─────────────── API HANDLERS (SAFE ERROR HANDLING) ─────────────── */
//   const handleFundWallet = async (e) => {
//     e.preventDefault();
//     const amount = Number(fundData.amount);
//     if (amount < 100) {
//       setFundResult({ type: "error", message: "Minimum deposit is 100 USDT" });
//       return;
//     }
...
//     setFundResult(null);
//     try {
//       const res = await initiateFund({ amount, payCurrency: fundData.payCurrency });
//       // Safe access: Check if res.data exists
//       if (res.data) {
//         setFundResult({ type: "success", data: res.data });
//       } else {
//         setFundResult({ type: "success", data: { message: "Payment initiated successfully!" } });
//       }
//       setFundData({ amount: "", payCurrency: "USDTBSC" });
//     } catch (err) {
//       console.error("Fund Error Details:", err);  // Debug log
//       setFundResult({
//         type: "error",
//         message: err.response?.data?.message || err.message || "Failed to initiate payment. Check network.",
//       });
//     } finally {
//       setFundLoading(false);
//     }
...
//     } else {
//       setTransferResult({ type: "success", data: { message: "Transfer successful!" } });
//     }
//     setTransferData({ toUserId: "", amount: "" });
//   } catch (err) {
...
//     setTransferResult({
//       type: "error",
//       message: err.response?.data?.message || err.message || "Transfer failed. Try again.",
//     });
//   } finally {
//     setTransferLoading(false);
//   }
...
//       setWithdrawalResult({
//         type: "success",
//         message: res.data?.message || "Withdrawal submitted successfully.",
//       });
//       setWithdrawalData({ amount: "", withdrawalAddress: "" });
//     } catch (err) {
//       console.error("Withdrawal Error Details:", err);  // Debug log
//       setWithdrawalResult({
//         type: "error",
//         message: err.response?.data?.message || err.message || "Withdrawal failed. Check details.",
//       });
//     } finally {
//       setWithdrawalLoading(false);
//     }
  ...
  const handleFundWallet = async (e) => {
    ...
    try {
      ...
      setPaymentStatus({
        ...
        transaction: { status: "waiting", amount },
      });
      setFundResult({ type: "success", data: res });
    } catch (err) {
      setFundResult({
