...
// /* ─────────────────────── API FUNCTIONS ─────────────────────── */
// const initiateFund = async (data) => {
//   const res = await apiClient.post("/api/users/wallet/initiate-fund", data);
//   return res.data;
// };

// const withdrawFunds = async (data) => {
//   const res = await apiClient.post("/api/users/withdrawal", data);
//   return res.data;
// };

// const transferFunds = async (data) => {
//   const res = await apiClient.post("/api/users/transfer", data);
//   return res.data;
// };

// /* ─────────────────────── DEPOSIT INFO MODAL ─────────────────────── */
// const DepositInfoModal = ({ onConfirm, closeModal }) => {
//   return (
//     <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
//       <motion.div
//         initial={{ scale: 0.9, opacity: 0 }}
...
//           <Info className="text-[#00A991]" size={24} />
//           <h2 className="text-xl font-bold text-gray-900 dark:text-white">Deposit Information</h2>
//         </div>
...
//           </p>
//           <p>
//             <strong>Cryptocurrency:</strong> USDT (Tether)
//           </p>
//           <p>
//             <strong>Minimum Deposit:</strong> 100 USDT
//           </p>
//           <p className="text-xs text-amber-600 dark:text-amber-400">
//             Deposits below 100 USDT will not be credited.
//           </p>
//         </div>
...
//           <button
//             onClick={onConfirm}
//             className="px-5 py-2 bg-[#00A991] text-white rounded-lg font-medium hover:bg-[#008a77]"
//           >
//             I Understand
//           </button>
//         </div>
...
//   const handleFundWallet = async (e) => {
//     e.preventDefault();
//     if (Number(fundData.amount) < MIN_AMOUNT) {
//       toast.error(`Minimum deposit is ${MIN_AMOUNT} USDT`);
//       return;
//     }
...
//       setFundResult({
//         type: "error",
//         message: err.response?.data?.message || "Failed to initiate deposit.",
//       });
//     } finally {
//       setFundLoading(false);
//     }
...
//   if (showInfo) {
//     return <DepositInfoModal onConfirm={handleConfirmInfo} closeModal={closeModal} />;
//   }
...
//                 className="absolute right-2 top-1/2 -translate-y-1/2 px-2 py-1 text-xs font-medium text-[#00A991] bg-[#00A991]/10 rounded hover:bg-[#00A991]/20 transition"
//               >
//                 Min
//               </button>
//             </div>
...
//                 <>
//                   <CheckCircle2 size={18} className="inline mr-2" />
//                   {fundResult.data.message || "Deposit initiated!"}
//                   {fundResult.data.payAddress && (
//                     <div className="mt-2 p-2 bg-white dark:bg-neutral-700 rounded border font-mono text-xs break-all">
//                       <p>
//                         Send <strong>{fundResult.data.payAmount} {fundResult.data.payCurrency}</strong> to:
//                       </p>
...
//         withdrawalAddress: withdrawalData.withdrawalAddress.trim(),
//       });
//       setWithdrawalResult({ type: "success", message: res.message || "Withdrawal request submitted." });
//       setWithdrawalData({ amount: "", withdrawalAddress: "" });
//     } catch (err) {
//       setWithdrawalResult({
//         type: "error",
      ...
      >
        ...
        <form onSubmit={handleFundWallet} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                value={fundData.amount}
                onChange={(e) => setFundData({ ...fundData, amount: e.target.value })}
                className="w-full px-3 py-2 pr-16 border border-gray-300 dark:border-neutral-600 rounded-2xl bg-white dark:bg-neutral-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-[#00A991] focus:border-transparent"
                placeholder="100"
                disabled={!!paymentId}
              />
              <button
          ...
          <div>
            ...
            <div className="p-2 bg-gray-100 dark:bg-neutral-700 rounded-2xl text-sm">
              Binance Smart Chain (BEP-20)
            </div>
          </div>

          <div>
            <label className="block text-sm mb-1">Cryptocurrency</label>
            <div className="p-2 bg-gray-100 dark:bg-neutral-700 rounded-2xl text-sm">
              USDT (Tether)
            </div>
          </div>
          ...
          {(fundResult || paymentId) && (
            <div className="p-4 bg-gray-50 dark:bg-neutral-700 rounded-2xl text-sm space-y-3">
...
export default function Transaction() {
  ...
  const handleFundWallet = async (e) => {
    ...
    try {
      const res = await apiClient.post("/api/users/wallet/initiate-fund", {
        amount,
        payCurrency: "USDTBSC",
      });

      const payId = res.data.paymentId || res.data.data?.paymentId;
      ...
      >
        ...
        {showDepositInfo && (
          <DepositInfoModal
            ...
            closeModal={() => setShowDepositInfo(false)}
          />
        )}
        {showDeposit && (
          <DepositModal
