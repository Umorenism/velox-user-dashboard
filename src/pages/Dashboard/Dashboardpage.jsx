...
// import { initiateFund, transferFunds,checkPaymentStatus } from "../../api/walletApi";
...
//         <button
//           onClick={onCancel}
//           className="px-4 py-2 bg-gray-300 dark:bg-neutral-600 rounded-md text-sm font-medium"
//         >
//           Cancel
//         </button>
//         <button
//           onClick={onConfirm}
//           className="px-4 py-2 bg-[#00A991] text-white rounded-md text-sm font-medium"
//         >
//           I Understand
//         </button>
//       </div>
...
//     waiting: { icon: <Hourglass size={14} />, color: "bg-orange-100 text-orange-700", label: "Waiting for Approval" },
...
//   useEffect(() => {
//     if (paymentId && !paymentStatus?.transaction?.status) {
//       pollInterval.current = setInterval(async () => {
//         try {
//           const token = localStorage.getItem("token") || sessionStorage.getItem("token");
//           const res = await axios.get(`/api/users/wallet/check-status/${paymentId}`, {
...
//     };
//   }, [paymentId, paymentStatus, setPaymentStatus]);

//   const handleMinClick = () => setFundData({ ...fundData, amount: "100" });

//   const currentStatus = paymentStatus?.transaction?.status || (paymentId ? "pending" : null);
...
//             <div className="p-3 bg-gray-50 dark:bg-neutral-700 rounded-md text-sm space-y-2">
//               {fundResult && !paymentId && (
//                 <>
...
//                     {fundResult.data.payAmount} {fundResult.data.payCurrency}
//                   </p>
//                   <p>To Address:</p>
//                   <p className="font-mono text-xs break-all">{fundResult.data.payAddress}</p>
//                   <p className="text-xs">
//                     Payment ID: <span className="font-mono">{fundResult.data.paymentId}</span>
//                   </p>
//                 </>
//               )}
...
//                     <p>
//                       Amount: <strong>{paymentStatus.transaction.amount} USDT</strong>
//                     </p>
//                   )}
//                   {currentStatus === "confirmed" && (
//                     <p className="text-emerald-700">
//                       <CheckCircle2 size={16} className="inline mr-1" />
//                       {paymentStatus.message || "Deposit confirmed!"}
//                     </p>
//                   )}
//                 </div>
//               )}
...
//   useEffect(() => {
//     const storedId = localStorage.getItem("pendingPaymentId");
//     if (storedId && !paymentStatus) {
//       setPaymentId(storedId);
//       setShowDeposit(true);
//     }
//   }, [paymentStatus]);

//   // Refresh profile on confirm
//   useEffect(() => {
//     const handleDepositConfirmed = () => {
//       const token = localStorage.getItem("token") || sessionStorage.getItem("token");
//       if (token) getUserProfile(token).then(setProfile).catch(console.error);
//     };
//     window.addEventListener("depositConfirmed", handleDepositConfirmed);
//     return () => window.removeEventListener("depositConfirmed", handleDepositConfirmed);
//   }, []);



//   // 🧠 Deposit Auto-Checker — runs every 5 minutes
// useEffect(() => {
...
//       if (!statusRes || !statusRes.transaction) return;

//       const { status, amount } = statusRes.transaction;

//       console.log(`🔁 Checking deposit status: ${status}`);

//       // ✅ WAITING
//       if (status === "waiting") {
//         // You could optionally show a "pending" message
//         return;
//       }

//       // ⚠️ PARTIALLY PAID
//       if (status === "partially_paid") {
//         console.warn("⚠️ Deposit partially paid");
//         // You can toast a message if using react-hot-toast
//         return;
//       }
...
//         // Remove pending payment ID
//         localStorage.removeItem("pendingPaymentId");

//         // Update wallet immediately
//         if (statusRes.user?.wallets?.deposit) {
//           setProfile((prev) => {
//             if (!prev) return prev;
//             return {
//               ...prev,
//               wallets: {
//                 ...prev.wallets,
//                 deposit: statusRes.user.wallets.deposit,
//               },
//             };
//           });
//         }
...
//         // Notify the user
//         alert(`✅ Payment confirmed! Your deposit of $${amount} has been credited.`);

//         // Trigger a global event if needed
//         window.dispatchEvent(new Event("depositConfirmed"));
//       }
//     } catch (err) {
...
//         setPaymentId(payId);
//         // Show initial pending status
//         setPaymentStatus({
//           message: "Payment initiated. Waiting for confirmation...",
//           transaction: { status: "pending", amount: Number(fundData.amount), paymentId: payId },
//         });
//       }
...
//           if (res.data.user?.wallets?.deposit) {
//             setProfile(prev => ({
//               ...prev,
//               wallets: {
//                 ...prev.wallets,
//                 deposit: res.data.user.wallets.deposit,
//               },
//             }));
//           }

//           // Show success banner
//           setSuccessMessage(`Deposit successful! $${transaction.amount} credited to your wallet.`);
...
//     // Resume pending deposit on page load
//     const storedId = localStorage.getItem("pendingPaymentId");
//     if (storedId && !paymentStatus) {
//       setPaymentId(storedId);
//       setShowDeposit(true);
...
//   // Refresh profile when deposit confirmed
//   useEffect(() => {
//     const handleDepositConfirmed = () => {
//       const token = localStorage.getItem("token") || sessionStorage.getItem("token");
//       if (token) {
...
//       <motion.div
//         initial={{ opacity: 0 }}
//         animate={showSuccessBanner ? { opacity: 1 } : { opacity: 0 }}
